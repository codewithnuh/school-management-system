...
class TeacherController {
    constructor() {
         ...
         */
        this.registerTeacher = async (req, res) => {
            try {
                ...
                const { cvPath, verificationDocument, ...teacherData } = teacher.toJSON();
         ...
         */
        this.acceptTeacherApplication = async (req, res) => {
        ...
        this.rejectTeacherApplication = async (req, res) => {
            const teacherId = req.query.id;
            if (teacherId === undefined || isNaN(Number(teacherId))) {
                res.status(400).json({ error: 'Invalid or missing teacher ID' });
                return;
        ...
        this.interviewTeacherApplicant = async (req, res) => {
            const teacherId = req.query.id;
            if (teacherId === undefined || isNaN(Number(teacherId))) {
                res.status(400).json({ error: 'Invalid or missing teacher ID' });
                return;
